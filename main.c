...
void printPlayerStatus(void)
{
     ...
     for(i=0;i<smm_player_nr;i++)
     {
         printf("%s - position:%i(%s), credit:%i, energy:%i\n", 
                ...
                smm_players[i].name, 
                smm_players[i].pos, 
                smmObj_getNodeName(smm_players[i].pos), 
...
void generatePlayers(int n, int initEnergy)
{
   ...
   for (i=0;i<n;i++)
   {
       ...
       printf("Input %d -th player name:", i);
...
{
     int type = smm_players[player].pos smmObj_getNodeType(smm_players[player]pos);
    switch(type)
    {
        //case lecture:
        default:
...
int main(int argc, const char * argv[]) {
    ...
    while ( fscanf(fp, "%s %i %i %i", name , &type, &credit, &energy) == 4 ) //read a node parameter set
    {
        ...
        smm_board_nr = smmObj_getNode(name, type, credit, energy);
    }
    fclose(fp);
